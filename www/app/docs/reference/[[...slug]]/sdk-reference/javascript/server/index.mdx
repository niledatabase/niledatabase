...
The query builder is built on top of [Knex](https://knexjs.org/), so most of the documentation for Knex applies to the query builder as well.
...
The query builder is available as `nile.db`. You can use it to query the database directly, or to build a model layer on top of it. It should work exactly like Knex.
