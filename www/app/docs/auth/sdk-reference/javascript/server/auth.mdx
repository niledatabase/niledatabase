...
| --- | --- | --- | --- |
| `token` | `string` | `undefined` | MFA challenge token issued during setup or sign-in; required for `scope: 'challenge'`. |
| `scope` | `'setup' \| 'challenge'` | `'challenge'` | Indicates whether you are starting setup or completing a challenge. |
| `method` | `'authenticator' \| 'email'` | `'authenticator'` | MFA mechanism to operate on. |
| `code` | `string` | `undefined` | Verification or recovery code when completing a challenge. |
| `remove` | `boolean` | `false` | Sends a `DELETE` request to remove the currently enrolled method. |
| `rawResponse` | `boolean` | `false` | When `true`, skip JSON parsing and return the raw `Response`. |
