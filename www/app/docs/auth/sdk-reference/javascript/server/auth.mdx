...
<Note>
On success, a session token is automatically stored via `setContext()` and used in subsequent calls.
</Note>
...
```ts raw
const res = await nile.auth.signIn('email', {
  email: 'jane@example.com',
  password: 'secure123',
}, true);
```
...
On success, the session and CSRF cookies are cleared, and `nile.setContext()` resets the current session state.
...
A `Response`. On success, session headers are automatically applied using `setContext()`.
...
Session headers are automatically managed via `nile.setContext()` after sign-in or sign-up. You generally don&apos;t need to set them manually.

```ts
nile.setContext({ tenantId: '0196...', userId: 'user-id' });
```

<Note>
Manual manipulation of `nile.api.headers` is discouraged unless absolutely necessary. Prefer `setContext()` to keep internal state consistent.
</Note>
