...
You can set the current tenant using `nile.setContext({ tenantId })`:

```ts
nile.setContext({ tenantId: '019612d7-56e7-7e87-8f30-ad6b05d85645' });
```

This method updates the internal configuration to use the given tenant in subsequent API calls.

<Note>
Framework-specific integrations may set context automatically, so calling `setContext` may not be necessary in those environments. see `link to extensions` for more information
</Note>
...
if (!(tenants instanceof Response)) {
  nile.setContext({ tenantId: tenants[0].id });
}
```
</CodeGroup>
...
```ts
nile.setContext({ tenantId: '0196...' });
const tenant = await nile.tenants.update({ name: 'New Name' });
```
...
```ts
nile.setContext({ tenantId: '0196...' });
await nile.tenants.addMember('user-id');
```
...
```ts
nile.setContext({ tenantId: '0196...' });
await nile.tenants.removeMember('user-id');
```
...
```ts
nile.setContext({ tenantId: '0196...' });
await nile.tenants.leaveTenant();
```
...
<CodeGroup>
```ts invite
await nile.tenants.invite('user@example.com');
```

```ts invite with callback
await nile.tenants.invite({ email: 'user@example.com', callbackUrl: 'https://myapp.com/welcome' });
```
