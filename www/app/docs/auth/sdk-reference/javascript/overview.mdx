...
It is important to ensure that the proper context is being used when calling functions from the server side. This can be done in [extensions](../../concepts/extensions.mdx) intelligently, or by using `nile.withContext`.
