...
Client-side MFA UI powered by `@niledatabase/react`. These components pair with `useMultiFactor` (and the low-level `mfa` helper) to enroll authenticator or email factors, verify challenges, and disable MFA when needed.
...
| `onRedirect` | `(url: string) => void` | `window.location.assign` | Handle `{ url }` redirect responses. |
...
- Redirects surface as `{ url: string }`; parse `error` from the query string for messaging.
