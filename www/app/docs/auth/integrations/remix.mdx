...
Remember that `@niledatabase/server` would be called in a loader or action, but `@niledatabase/client` would be used client side. 
...
export const loader: LoaderFunction = async ({ request }) => {
  try {
    nile.setContext(request.headers);// be sure the context is correct from the request
