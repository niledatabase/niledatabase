...
Client-side MFA enrollment, challenge, and removal flows powered by the React SDK.
...
### Overview
Nile Auth exposes a single `/auth/mfa` endpoint for starting MFA setup, completing challenges, and removing an enrolled method. The React SDK wraps the endpoint in `useMultiFactor` and UI components that parse server responses into ready-to-render experiences. For custom prompts, you can call the `mfa` helper from `@niledatabase/client` directly; it understands the same payloads and returns either redirect URLs or structured setup/challenge objects.

Tokens returned from setup or sign-in responses must be echoed back on subsequent calls (challenge verification or removal). When the backend needs the browser to redirect, the helper returns `{ url: string }` so you can route accordingly. HTTP verbs mirror the API contract: `POST` initiates setup, `PUT` verifies a challenge (token + code), and `DELETE` disables MFA when `remove: true` is passed.
...
async function verify({ token, code, method }: { token: string; code: string; method: 'authenticator' | 'email'; }) {
  const result = await mfa({ token, code, method, scope: 'challenge' });
  if (result?.ok) {
    // `scope` will be "challenge" for sign-in or "setup" when completing enrollment
    window.location.replace('/app');
  } else if (result && 'url' in result) {
    const url = new URL(result.url);
    throw new Error(url.searchParams.get('error') ?? 'Unable to verify code');
  }
}
```
...
| `onRedirect` | `(url: string) => void` | `window.location.assign` | Optional handler when the backend responds with a `url` to visit. |
...
- `MultiFactorChallenge` — shared challenge UI for either method (used for disable flows or sign-in prompts).  
  Props: `setup: MfaSetup`, `message: string`, `isEnrolled: boolean`, `onSuccess(scope)`.

### `mfa(options)` (client helper)
Low-level wrapper around `/auth/mfa`. Verbs follow the OpenAPI spec:
- `POST` with `{ method, scope: 'setup' }` to initiate setup and receive a challenge (201 on success).
- `PUT` with `{ token, code, method, scope }` to satisfy a challenge (200 on success; 400 invalid payload; 401 invalid code; 404/410 missing or expired challenge).
- `DELETE` with `{ token?, code?, method, scope? }` when `remove: true` to disable MFA; may require a valid code/token if the backend is configured to require confirmation.

| Name | Type | Default | Description |
| --- | --- | --- | --- |
| `token` | `string` | `undefined` | Challenge token issued during setup or sign-in; required for verification and some disable flows (email). |
| `scope` | `'setup' \| 'challenge'` | `'challenge'` | Operation to perform; setup responses can still return `scope: 'challenge'` when verification is needed. |
| `method` | `'authenticator' \| 'email'` | `'authenticator'` | MFA mechanism to target. |
| `code` | `string` | `undefined` | 6-digit code or recovery code for verification/disable. |
...
| `init` | `RequestInit` | `undefined` | Extra fetch options (e.g., credentials). |
...
- `{ url: string }` when the backend expects a redirect (errors are provided via `error` query param).

### Error handling
- Non-200 responses are coerced into `{ url: string }` with an `error` search param. `MfaVerifyForm` and the examples above parse this for user-friendly messaging.
