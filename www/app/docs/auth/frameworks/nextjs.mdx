...
```bash npm
  npm install @niledatabase/server @niledatabase/nextjs @niledatabase/react
```
```bash yarn 
  yarn add @niledatabase/server @niledatbase/nextjs @niledatabase/react
```
```bash pnpm
  pnpm add @niledatabase/server @niledatabase/nextjs @niledatabase/react
```
</CodeGroup>
</Step>
...
<Info>`nile.users.verifySelf` mirrors the client-side call to `/api/auth/verify-email`. All server side calls can be called from a client based on their configured route.</Info>
