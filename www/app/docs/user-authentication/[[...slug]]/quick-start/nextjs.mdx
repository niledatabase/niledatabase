...
If you haven't already, [sign up for Nile](https://console.thenile.dev) and follow the prompts to create a workspace and database.
...
In the project root, rename `.env.local.example` to `.env.local`, and open it for editing. You'll provide the `NEXT_PUBLIC_WORKSPACE` and `NEXT_PUBLIC_DATABASE` values. You can get these from the [Nile Console](https://console.thenile.dev)
