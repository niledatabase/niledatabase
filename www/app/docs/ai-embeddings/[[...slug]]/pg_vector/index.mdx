...
And these can be set to `strict_order` or `relaxed_order`. Strict order guarantees that results returned are ordered by exact distance.
...
If you use relaxed ordering, you can re-order the result using materialized CTE:
