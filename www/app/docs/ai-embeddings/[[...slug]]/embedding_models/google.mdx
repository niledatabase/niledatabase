...
const doc_vec_resp = await embed.embedContent({
  ...
  taskType: "RETRIEVAL_DOCUMENT", // optional. used for embeddings of documents stored in pg_vector
  title: "" // optional title, can be used with RETRIEVAL_DOCUMENT and possibly improve the quality of the embeddings
});
...
const question_vec_resp = await embed.embedContent({
    ...
    taskType: 'RETRIEVAL_QUERY', // used for embeddings of questions used to search documents
});

// Google returns a response object with an embedding field that contains the embeddings in the value field
...
await nile.db.query("CREATE TABLE IF NOT EXISTS embeddings (embedding vector(768))");
