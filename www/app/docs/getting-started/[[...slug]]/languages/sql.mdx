...
FROM todoscorrect join tenants on tenants.id = todoscorrect.tenant_id;
```
...
insert into todos (tenant_id, title, estimate, embedding, complete) values ('7e93c45f-fe65-4f26-8ab6-922850fa4c7a', 'take out the trash', '2h', '[2,3,4]', false);
...
-------------------------------------------------------------------------------------
my first customer    feed my cat           [1,2,3]             1h             false
second customer   take out the trash       [2,3,4]             2h             false
```
...
## 7. Inserting second todo item for the second customer using vector similarity and enforcing tenant isolation

The embedding for the new todo task is calculated first from the LLM (let's say this is [2,3,4]). We will get similar tasks for the second customer based on the new task's embedding. 
...
SELECT title, estimate FROM todos WHERE embedding <-> '[2,3,4]' < 1;
```
title               estimate       complete
-------------------------------------------------------------------------------------
take out the trash    2h             false
...
```sql
-- a new todo item for our second tenant
insert into todos (tenant_id, title, estimate, embedding, complete) values ('7e93c45f-fe65-4f26-8ab6-922850fa4c7a', 'clean the house', '1.5h', '[2,3,4]', false);
...
-------------------------------------------------------------------------------------
second customer   take out the trash       [2,3,4]             2h             false
second customer   clean the house          [2,3,4]             2h             false
```
