...
Lets imagine that we are building a todo list app that maintains task for each tenant/customer and also uses AI to automatically predict the time required to complete the task.
...
We will use some synthetic value for the vector embeddings. In the real world, you would get the embeddings from one
of the model providers.

```sql
-- adding a todo item for this tenant. We calculate the 
insert into todos (tenant_id, title, estimate, embedding, complete) values (d24419bf-6122-4879-afa5-1d9c1b051d72, 'feed my cat', '2h', [1,2,3], false);
