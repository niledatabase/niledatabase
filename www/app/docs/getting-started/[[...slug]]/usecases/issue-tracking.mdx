...
**Description:** Stores information about each project. The application can support multiple projects.
...
**Description:** Tracks the status changes for each task. The status of each task can also be converted to embeddings if they need to be searched using AI models.
...
**Description:** Stores comments added to each task. The embeddings are created for each comments. Typically, comments are going to be short under tasks and one embedding per comment should be a good chunking length.
...
**Description:** Contains tasks from across projects for global planning.
