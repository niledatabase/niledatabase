...
✏️ The examples use `--rm`, so Docker removes the container and its anonymous data volumes when it exits.
Run a new container to start with a fresh database. Stopping a container without `--rm` does not remove its data.
...
```bash
docker run --pull=always --rm -p 5432:5432 -ti \
...
To test email-based authentication, see [Configuring email for local tests](/getting-started/postgres_testing#configuring-email-for-local-tests).
...
Run `docker pull ghcr.io/niledatabase/testingcontainer:latest`, then create a new container from that image.
Pulling an image does not update an existing container. The examples above use `--pull=always` to check for updates each time you run them.
...
- Change the port mapping when running the container. For example to port 5433, and then connect to the new port:
  ```bash
  docker run --pull=always --rm -p 5433:5432 -ti ghcr.io/niledatabase/testingcontainer:latest
