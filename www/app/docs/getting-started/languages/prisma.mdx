---
title: 'Build AI-Native B2B application with Postgres and Prisma'
sidebarTitle: 'Prisma'
---
...
<Step title="Check the data in Nile">
  ...
  ```sql SELECT tenants.name, title, estimate, complete FROM todos join tenants
  on tenants.id = todos.tenant_id; ``` You should see all the todos you created,
