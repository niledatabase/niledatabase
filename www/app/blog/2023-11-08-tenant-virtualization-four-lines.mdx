export const metadata = {
  ...
  sizzle:"Nile's serverless Postgres promises easy tenant isolation with a single database experience, requiring minimal code. Doubtful? A quick code walkthrough will prove it.",
...
console.log("showing todos for user " + tenantNile.userId + 
    " for tenant " + tenantNile.tenantId);
...
Therefore, we must **guarantee** the queries that are run execute on the *exact* same connection that has the context. 
...
```tsx
export function configureNile(rawAuthCookie: any, 
                tenantId: string | null | undefined)  {
    const authData = 
        JSON.parse(rawAuthCookie.value) as AuthCookieData;
