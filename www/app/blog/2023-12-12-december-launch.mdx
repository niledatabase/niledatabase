export const metadata = {
  title: "Announcing Nile's Behdet release - user management, invitations, and more",
  authors: ["gwenshap"],
...
- [Template](https://www.thenile.dev/templates/Todo%20List%20application%20backend%20with%20Nile%20and%20AWS%20Lambda) and [documentation o](https://www.thenile.dev/docs/serverless/lambda)n how to use Nile from AWS Lambda. The template uses the Serverless framework and Drizzle ORM to make this a true one-liner.
- [Template](https://www.thenile.dev/templates/Todo%20List%20with%20NextAuth-based%20Authentication) and [documentation](https://www.thenile.dev/docs/user-authentication/third-party/nextauth) on how to use Nile with NextAuth. We show off our integration with both social logins (Google) and Passwordless (via email). The template includes a Nile adapter that we are contributing back to the NextAuth project.
