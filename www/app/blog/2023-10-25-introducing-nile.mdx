...
standard table if you don’t specify any context.** ```sql select * from employees
```
<table>
  <thead>
    <tr>
...
and shared tables ```sql -- list of flights that a corporate travel booking site
can use to share across all the tenants. -- employees in different tenants can view
these flight data to book tickets create table flights ( id integer PRIMARY KEY,
name text, from_location text, to_location text, departure_time TIMESTAMP, arrival_time
TIMESTAMP); ```
