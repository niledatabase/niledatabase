export const metadata = {
  ...
  sizzle: "I am thrilled to announce Nile, a serverless Postgres database designed for modern SaaS applications.",
...
While there’s a need for virtual tenant DBs that isolate data, there’s also a need to [share data across tenants](https://www.thenile.dev/docs/tenant-sharing). It should be easy to create shared tables that can be accessed by/[across] all tenants and are globally available. All standard queries should work seamlessly both on tenant-aware and shared tables
```sql
