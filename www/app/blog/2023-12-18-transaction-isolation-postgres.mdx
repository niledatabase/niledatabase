...
The main problem with non repeatable reads is a classic anti-pattern known as “write after read”. For example, lets say that we want to give 10% bonus to accounts with balance above 1000, and we use the following pre:
...
You can find the [pre for the test suite in github](https://github.com/postgres/postgres/tree/master/src/test/isolation) and the README explains it well.
...
- [Blog post explaining atomicity, isolation and MVCC in Postgres with snippets from Postgres source pre](https://brandur.org/postgres-atomicity)
