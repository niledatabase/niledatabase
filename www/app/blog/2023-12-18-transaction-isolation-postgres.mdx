...
![“What, me worry?”](/blog/2023-12-18-transaction-isolation/whatmeworry.png)
...
The theoretical problem is that the transactions are not isolated and not serializable. Different results based on which transactions ran in parallel are now expected, which means that through testing is now more complex.

But the main problem with non repeatable reads is a classic anti-pattern known as “write after read”. For example, lets say that we want to give 10% bonus to accounts with balance above 1000, and we use the following code:
...
In isolation level “repeatable reads” both queries in session B will show the same balance for account 789. Because the `update` that was done in session A will not be visible to session B.  However, the second query will also include account 12345 that was created by session A. A new row suddenly appeared in the middle of a transaction.

This means that session B will produce an inconsistent report - both because it has two queries that returned different results, and because the second query shows the data that session A inserted, but not the data it updated.
...
You can find the code for the test suite here: https://github.com/postgres/postgres/tree/master/src/test/isolation and the README is great.

The core idea is simple and brilliant, like many things in Postgres:
The testers define different scenarios, just like the ones we demonstrated above. Session A does `update` and then `commit`, Session B does `select` and then `update`. You also specify the result you'd expect after all sessions and all transactions committed.

The test framework then takes all the sessions and runs all possible ways these could interleave. One such run can be “A updates, then B selects, then A commits, then B updates”. Another can be “B selects, then A updates, then B updates, then A commits”. It then checks that the key invariants like “account 1234 has a balance of 500” are preserved in all permutations.
