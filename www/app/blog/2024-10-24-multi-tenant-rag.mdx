...
However, behind the scenes, Nile isolates the tenants to their own virtual tenant databases and handles the routing of queries to the right destination. This allows us to enforce this isolation as well as additional capabilities such as placing tenants on dedicated or shared compute. In order to provide these capabilities, Nile enforces rules such as "every insert must specify the tenant ID" and "mixing different tenants in one transaction is not allowed", these rules are guardrails that help developers build secure and private applications.
...
If you think back at the examples of multi-tenant RAG applications at the beginning of this blog - Zenefits, Hubspot, Notion - they all combine RAG with application data in seamless user experience. Using Postgres to store both application data and vectors is a simple and efficient way to implement the same experience.

Lets take a look at a few examples of using Nile for multi-tenant RAG. In all the examples, we are building a smart task manager that uses past tasks, their time estimates and an LLM to estimate how long new tasks will take. To implement this, we insert tasks with embeddings and later search for similar tasks based on their embeddings. Naturally, we don't want to mix tasks between tenants. Lets see how to do it in a variety of languages and frameworks.
