export const metadata = {
  ...
  authors: ["gwenshap"],
  image: "2025-03-14-announcing-extension-store/extensions_in_console.png",
  sizzle:
...
PostgreSQL is more than just a database; it is a robust and extensible platform on which developers can build powerful applications.
At Nile, we've expanded our support for PostgreSQL extensions, enabling new use cases and providing greater flexibility 
for multi-tenant applications. This includes:
...
For example, here's how to create a tenant-aware product recommendation system:
...
Extensions like HLL, Quantile, Xicor and Financial empower applications that process large-scale financial transactions, 
real-time risk analysis, and portfolio optimizations.   

For example, here's how to analyze trading data with statistical measures:

```sql
-- Create a table for storing trading data
...
INSERT INTO trading_data (tenant_id, symbol, price, volume, timestamp) VALUES
(
    '123e4567-e89b-12d3-a456-426614174000',
    'STOCK_A',
    105.50,
...
-- Analyze trading patterns using various statistical measures
SELECT 
    symbol,
    count(*) as num_trades,
    avg(price) as avg_price,
    percentile_cont(0.5) WITHIN GROUP (ORDER BY price) as median_price,
    percentile_cont(0.95) WITHIN GROUP (ORDER BY volume) as volume_95th_percentile,
    xicor(price, volume) as price_volume_correlation,  -- Correlation between price and volume
    hll_cardinality(hll_add_agg(hll_hash_integer(volume))) as approximate_unique_volumes  -- HLL distinct count
FROM trading_data
WHERE 
    tenant_id = '123e4567-e89b-12d3-a456-426614174000'
GROUP BY symbol;
...
SELECT 
    ...
    volatility_3,
    CASE 
        WHEN price > moving_avg_3 THEN 'ABOVE_MA'
        WHEN price < moving_avg_3 THEN 'BELOW_MA'
        ELSE 'AT_MA'
