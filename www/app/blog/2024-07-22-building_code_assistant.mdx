export const metadata = {
  title: "AI Code Assistant built on GPT-4o-mini, Langchain, Postgres and pg_vector",
  authors: ["gwenshap"],
...
**In the Code Assistant architecture, I separated the two phases.** Updating Code Assistant with new repositories is done from a small script in the command line. 
While interrogating the repositories is done from a SaaS-y web UI. 

Within this common high-level design, there are many small decisions that make a large difference in performance and user experience. 
In this blog I'll walk you through the major decisions, explain how I made them, and show how they are reflected in the implementation. By the time you are done reading
(get some coffee, it isn't short), you'll know everything you need to build your own AI Code Assistant, or similar RAG-based SaaS.

We are going to go over the application from the ground up - from the database schema to the user interface.
...
Few noteworthy things in the design of this table:

**The table has a tenant_id column:** 
Which makes the table **tenant aware** - the embeddings for each tenant are stored in their own virtual tenant database and are isolated from each other. 
...
Note that the size of the model has to match the size of the vector column *exactly*. You can't store 2048 dimension vectors in a 1024 dimension column, but you also can't 
...
At this scale, vector indexes don't add much value (full scan of 100 blocks is fast) but they add overhead and potential loss of recall. So, I skipped them for now. We'll 
...
![Diff of switching embedding model](/blog/2024-07-22-building_code_assistant/change_model_diff.png)
...
But I have to say that in retrospect, I am not sure I’d have used it again.
...
```javascript
  ...
  try {
    for (const file of files) {
      ...
      if (content.length > 0 && content.length < 8192) { // skip files that are larger than the max-token limit of the model
        const result = await nile.db.query( // store file
        ...
        await client.query( // store embedding
...
If needed, we roll back the entire operation and re-try later. This is very useful while experimenting.
...
Note that the prompt is quite extensive. It took quite a few experiments to get it right, and I was surprised how much the prompt affected the quality of the response.
...
Looking at the LangChain example for working with a streaming response, you see something like this (OpenAI is very similar):
           ...
           while (!done) {
               ...
               if (!recievedFiles) {
                   ...
                   if (dataParts.length > 1) {
                       ...
                       dispatch({ type: 'updateAnswer', text: dataParts[1]}); // display first part of the answer
                       recievedFiles = true;
                   }
               } else {
                   dispatch({ type: 'updateAnswer', text: chunkValue}); // display another part of the answer
                   if (done) {
...
But until we add all these bells an whistles, you are welcome to try out the [Code Assistant](https://www.thenile.dev/templates/Multi-tenant%20Code%20Assistant%20built%20using%20Nile%2C%20pg_vector%20and%20OpenAI), 
