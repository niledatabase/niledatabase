...
# Setup the Handler Route

Install our Next.js starter from the command line:

```bash
npx create-next-app -e https://github.com/niledatabase/niledatabase/examples/handling-auth-nextjs handling-auth-nextjs && cd handling-auth-nextjs
```
...
When an error occurs, users will land on [this page.](http://localhost:3000/oops)

# Summary

Your application may have specific requirements around handling auth. This guide shows you the
general pattern for handling authentication attempts from Nile: determine the auth event, ensure
the request is valid, and redirect to the appropriate page.
