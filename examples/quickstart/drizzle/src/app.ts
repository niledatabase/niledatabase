...
import { tenants as tenantSchema, todos as todoSchema, users as usersSchema, tenant_users } from './db/schema';
...
async function dbAuthorizer(username: string, password: string, cb: (err: any, result: boolean) => void) {
  console.log('authenticating user: ' + username);
  if (!REQUIRE_AUTH) {
    return cb(null,true);
  }
  const users:any =  await tenantDB(async (tx) => {
    return await tx.select().from(usersSchema).where(eq(usersSchema.id, username));
  })
  ...
  try {
    ...
    if (REQUIRE_AUTH) {
      ...
      await tenantDB(async(tx) => {
        // @ts-ignore
        return await tx.insert(tenant_users).values({tenant_id: tenants[0].id, user_id: req.auth.user});
      });
    }
...
app.get("/api/tenants", async (req, res) => {
  ...
  try {
      if (REQUIRE_AUTH) {
       tenants = await tenantDB(async(tx) => {
        return (db.select().from(tenantSchema)
