...
async function indexPDFAction(formData: FormData) {
  ...
  console.log(
    "indexing pdf: " + rawFormData.url + " for tenant: " + rawFormData.tenantId
  );
...
export default async function Page() {
  ...
  const tenantDB = configureNile(cookies().get("authData"), tenantID);

  const fileSummary = await tenantDB.db.query(
              ...
              <form name="newtenant" id="newtenant" action={indexPDFAction}>
                <Stack spacing={3}>
                  <Typography level="title-lg">
                    Please provide a URL of a PDF
                  </Typography>
                  <Typography level="body-md">
                    We are going to chunk the document, generate embeddings and
                    store them. You will see, in the table below how long it
                    took to generate and store embeddings. For efficiency and
                    cost reasons, we are going to only index the first 10,000
                    characters of the document{" "}
                  </Typography>

                  <Typography level="body-md">
                    For example, you can try this short article about Phil
                    Jackson:
                    https://uhra.herts.ac.uk/bitstream/handle/2299/1346/900740.pdf
                  </Typography>
