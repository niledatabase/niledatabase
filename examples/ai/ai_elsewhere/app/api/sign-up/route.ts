...
import {generateTenant} from '@/lib/random_tenant';
...
export async function POST(req: Request) {
    ...
    if (res && res.status >= 200 && res.status < 300) {
        ...
        // also create a new tenant for the user
        nile.token=accessToken;
        const tenantRaw = await nile.api.tenants.createTenant({name: generateTenant()} );
        const tenant = await tenantRaw.json();
        console.log("created tenant: " + tenant);
        cookies().set('tenantId', tenant.id, cookieOptions(3600));
